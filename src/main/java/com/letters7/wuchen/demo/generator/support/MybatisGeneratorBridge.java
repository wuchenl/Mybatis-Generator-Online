...
/**
 * @author wuchen 2018-12-27 09:15:32
 * 用于操作Mybatis-Generator 生成对应代码
 */
...
//	    configuration.addClasspathEntry(connectorLibPath);
        ...
        if (StringUtils.isNotEmpty(generatorConfig.getGenerateKeys())) {
            ...
            if (DBType.MySQL5.name().equals(dbType2) || DBType.MySQL8.name().equals(dbType)) {
                dbType2 = "JDBC";
        ...
        // lombok插件
        if (generatorConfig.getUseLombok() != null && generatorConfig.getUseLombok()) {
            PluginConfiguration pluginConfiguration1 = new PluginConfiguration();
            pluginConfiguration1.addProperty("type", "com.letters7.wuchen.demo.generator.support.plugins.LomBokPlugin");
            pluginConfiguration1.setConfigurationType("com.letters7.wuchen.demo.generator.support.plugins.LomBokPlugin");
            context.addPluginConfiguration(pluginConfiguration1);
        }
        // limit/offset插件
        ...
        if (generatorConfig.isNeedForUpdate()) {
            if (DBType.MySQL5.name().equals(dbType)
                    || DBType.PostgreSQL.name().equals(dbType)) {
                PluginConfiguration pluginConfiguration = new PluginConfiguration();
